...
namespace TextStrings
{
    ...
    public static class Chatbot_TextStrings
    {
        ...
        public static class HelpCommandsStrings
        {
            ...
            public static string ErrorLink = "[SWSH] 1) Using correct trade command? (!strade), 2) Sending commands in correct order? (!setcode first, then !strade), 3) Connected to the Internet? (Y-Comm -> (+) Connect to the Internet), 4) Searching with saved Link Code? (Link Trade -> Set Link Code), 5) Try saving a more unique Link Code, 6) Try searching early before your turn is announced, 7) Try restarting your Switch";
            ...
            public static string ErrorInvalid = "If you are using a Showdown set, make sure you are not putting 'Request:' in your command. Otherwise, the bot is cancelling the trade because your customizations are not legal. Common issues include requesting an invalid ability, ball, level, and/or moveset, so try changing/removing these customizations first.";
