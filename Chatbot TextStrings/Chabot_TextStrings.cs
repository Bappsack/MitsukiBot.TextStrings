...
namespace TextStrings
{
    ...
    public static class Chatbot_TextStrings
    {
        ...
        public static class HelpCommandsStrings
        {
            ...
            public static string ErrorUnion = "[BDSP] 1) Using correct trade command? (!tradeBDSP), 2) Sending commands in correct order? (!setcode first, then !tradeBDSP), 3) Using Global Room?, 4) Searching with saved Link Code? (Global Room -> Yes, with a Link Code), 5) Try searching early before your turn is announced, 6) Try restarting your Switch";
            public static string ErrorLink = "[SWSH] 1) Using correct trade command? (!strade), 2) Sending commands in correct order? (!setcode first, then !strade), 3) Connected to the Internet? (Y-Comm -> (+) Connect to the Internet), 4) Searching with saved Link Code? (Link Trade -> Set Link Code), 5) Try searching early before your turn is announced, 6) Try restarting your Switch";
            public static string ErrorGTS = "[3DS] 1) Using correct trade command? (!tradeORAS for Gen 6, !trade for Gen 7), 2) Saved correct friend code? (!myfc), 3) Specified correct Pokémon in 'Deposit: [Pokémon]' command? (Pokémon deposited, not Pokémon wanted), 4) Specified deposit Pokémon if using Showdown set? (Add 'Deposit: [Pokémon]' after first line), 5) Try using a different deposit Pokémon, 6) Make sure you aren't requesting a mythical Pokémon in Gen 7!";
            public static string ErrorInvalid = "If your customizations are not valid, the bot will cancel the trade. Common issues include requesting an invalid ability, ball, level, and/or moveset, so try changing/removing these customizations first. If you require further assistance, ask for help in the #support channel in our Discord server!";
